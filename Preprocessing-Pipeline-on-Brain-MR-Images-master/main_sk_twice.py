...
if __name__ == '__main__':
    ...
    # input and anli
    input_anli_path = '/home/zhenghuadong/project/Preprocessing-Pipeline-on-Brain-MR-Images-master/data/anli/'
    output_anli_path = './data/anli_prep_sk_twice'
    ...
    for anli_dir in input_anli_list:
        ...
        for regFile in regFiles:
            ...
            dstFilePath = dstFile.as_posix()
            logging.info('1 Stripping on : {}'.format(regFilePath))
            try:
                # print('test')
        ...
        stripFiles_2 = list()
        for regFile in stripFiles:
            regFilePath = regFile.as_posix()
            # dstFile = regFile.parent / (regFile.stem.split('.')[0] + '_strip.nii.gz')
            dstFile = Path(output_anli_dir_path) / (os.path.basename(regFile))
            stripFiles_2.append(dstFile)
            dstFilePath = dstFile.as_posix()
            logging.info('2 Stripping on : {}'.format(regFilePath))
            try:
                # print('test')
                bet(regFilePath, dstFilePath, frac=0.3)
            except RuntimeError:
