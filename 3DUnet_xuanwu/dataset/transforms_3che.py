...
class RandomCrop:
    ...
    def __call__(self, img, mask):
        ...
        ss_z, es_z = self._get_range(mask.size(1), self.slices)
        ss_y, es_y = self._get_range(mask.size(2), 160)
        ss_x, es_x = self._get_range(mask.size(3), 160)
        # ss, es = self._get_range(mask.size(1), self.slices)
        
        # print(img.shape, mask.shape)
        # tmp_img = torch.zeros((img.size(0), img.size(1), img.size(2),  self.slices))
        # tmp_mask = torch.zeros((mask.size(0), mask.size(1), mask.size(2), self.slices))
        #
        tmp_img = img[ :, ss_z:es_z, ss_y:es_y, ss_x:es_x]
        tmp_mask = mask[ :, ss_z:es_z, ss_y:es_y, ss_x:es_x]
        # print('tmp_img.shape', tmp_img.shape, 'tmp_mask.shape', tmp_mask.shape)
